...
mod tests {
    ...
    use solana_program::{
        pubkey::Pubkey,
        system_instruction::transfer, example_mocks::solana_sdk::transaction, system_program
    };
    use bs58;
    ...
    fn keygen() {
        ...
        println!("");
        println!("To save your wallet, copy and paste the JSON file:");
        println!("{:?}", kp.to_bytes());
    } 

    #[test] 
    fn airdop() {
        let keypair = read_keypair_file("wba-wallet.json").expect("Couldn't find wallet file");
        let client = RpcClient::new(RPC_URL);
        match client.request_airdrop(&keypair.pubkey(), 2_000_000_000u64) {
            Ok(s) => {
                println!("Success! Check out your TX here:");
                println!("https://explorer.solana.com/tx/{}?custer=devnet", s.to_string());

            },
            Err(e) => println!("Ooops, something went wrong: {}", e.to_string())
        }
    } 
    ...
    fn transfer_sol() {
        let keypair = read_keypair_file("wba-wallet.json").expect("Couldn't find wallet file");
        let to_pubkey = Pubkey::from_str("8QhnBerkJRyzyEe6qNudWPEAoGFHvE7V2xTPJxDyNdkh").unwrap();
        let rpc_client = RpcClient::new(RPC_URL);
        let balance = rpc_client.get_balance(&keypair.pubkey()).expect("Failed to get balance");
        let recent_blockhash = rpc_client.get_latest_blockhash().expect("Failed to get recent blockhash");
        let message = Message::new_with_blockhash(
            &[transfer(
                &keypair.pubkey(), &to_pubkey, balance
            )], 
    ...
    fn enroll() {
        ...
        let args = CompleteArgs {
            github: b"anthonyliriano".to_vec()
        };
